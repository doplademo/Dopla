	...
	<main>
		...
		<section class="px-4 pb-12 bg-greenHover">
			<prescription-item v-for="prescription in prescriptionsOther" :name="prescription.name"
				:id="prescription.id" :selected="prescription.selected" :key="prescription.id"
				@toggle="onSelectOther" />
		</section>
		<modal-container v-if="showModal">
			<info-modal :product="infoProduct" @on-option-select="onChangeOption" @add="add" @subtract="subtract"
				@close="closeModal" />
		</modal-container>
	</main>
</template>
